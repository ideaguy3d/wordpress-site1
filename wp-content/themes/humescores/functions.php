...
// this is for child themes, if there is a func "humescores_setup" it'll overwrite the parent theme
if (!function_exists('humescores_setup')) :
    ...
    function humescores_setup() {
        ...
        // Set up the WordPress core custom background feature. This is used in the customizer.
...
// this will run after the "after_setup_theme" event fires from wordpress
...
function humescores_fonts_url() {
    ...
    // if either array has the key 'on' do something
...
function humescores_resource_hints($urls, $relation_type) {
    if (wp_style_is('humescores-fonts', 'queue')
 ...
 *
 * This function is for when we embed external content like a YouTube vid
 *
 * @global int $content_width
 */
function humescores_content_width() {
    // e.g. the max width of a YouTube vid will be 640.
 ...
 * Even though it's "register_sidebar()" widgets are not always in the sidebar anymore.
 *
...
function humescores_widgets_init() {
    register_sidebar(array(
        ...
        // %1$s and %2$s are both refering to the id "sidebar-1" from above code
 ...
 * This is where we add additional css and js instead of adding them to the header or footer.
 */

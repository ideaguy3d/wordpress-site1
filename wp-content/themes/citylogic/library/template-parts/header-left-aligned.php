...
if ( function_exists( 'has_custom_logo' ) ) {
	...
	$logo = "<a href=\"". esc_url( $logo_link_content ) ."\" title=\"". esc_attr( get_bloginfo( 'name', 'display' ) ) ."\" class=\"custom-logo-link\"><img src=\"". esc_url( get_theme_mod( 'citylogic-logo' ) ) ."\" alt=\"". esc_attr( get_bloginfo( 'name' ) ) ."\" class=\"custom-logo\" /></a>";
}
?>
...
$is_navigation_menu_transparent = $navigation_menu_opacity == 0;

if ( ( $is_navigation_menu_translucent || $is_navigation_menu_transparent ) && !$show_slider && !$show_header_image ) {
	$is_navigation_menu_translucent = false;
	$is_navigation_menu_transparent = false;
}
?>

<nav id="site-navigation" class="main-navigation border-bottom rollover-underline centered-submenu <?php echo esc_attr( get_theme_mod( 'citylogic-navigation-menu-alignment', customizer_library_get_default( 'citylogic-navigation-menu-alignment' ) ) ); ?> <?php echo ( $is_navigation_menu_translucent && !$mobile_sticky_disabled ) ? 'translucent' : ''; ?> <?php echo ( $is_navigation_menu_transparent && !$mobile_sticky_disabled ) ? 'transparent' : ''; ?>" role="navigation">
		...
		<div class="search-slidedown">
			<div class="container">
				<div class="padder">
					...
					<?php if ( get_theme_mod( 'citylogic-navigation-menu-search-button', customizer_library_get_default( 'citylogic-navigation-menu-search-button' ) ) ) : ?>
						<?php get_search_form(); ?>
					<?php endif; ?>
					</div>
				</div>
